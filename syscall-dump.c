...
#ifdef SYS_gettid
static inline int gettid(void) { return syscall(SYS_gettid); }
#else
...
static FILE *(*real_freopen)(const char *pathname, const char *mode,
                             FILE *stream);
...
static void dlog(const char *fmt, ...) {
  ...
  if (dlog_fd < 0) {
    ...
    dlog_fd =
        f ? real_open(f, O_CREAT | O_APPEND | O_WRONLY, 0600) : 2 /* stderr */;
    if (dlog_fd < 0)
      exit(1002);
...
FILE *fopen(const char *pathname, const char *mode) {
  ...
  dlog("fopen('%s', '%s') = %p (%d) (fileno %d)\n", pathname, mode, r,
       r == NULL ? errno : 0, r != NULL ? fileno(r) : -1);
  errno = keep_errno;
  return r;
}
...
FILE *freopen(const char *pathname, const char *mode, FILE *stream) {
  ...
  dlog("freopen('%s', '%s', %p) = %p (%d) (fileno %d)\n", pathname, mode,
       stream, r, r == NULL ? errno : 0, r != NULL ? fileno(r) : -1);
  errno = keep_errno;
  return r;
}
...
int bind(int socket, const struct sockaddr *address, socklen_t address_len) {
  ...
  if (address->sa_family == AF_UNIX)
    ...
    dlog("bind AF_INET to '%s:%d'\n",
         inet_ntop(AF_INET, &(((struct sockaddr_in *)address)->sin_addr), s,
                   sizeof(s)),
         ((struct sockaddr_in *)address)->sin_port);
  else if (address->sa_family == AF_INET6)
    dlog("bind AF_INET to '%s:%d'\n",
         inet_ntop(AF_INET6, &(((struct sockaddr_in6 *)address)->sin6_addr), s,
                   sizeof(s)),
         ((struct sockaddr_in6 *)address)->sin6_port);
  r = real_bind(socket, address, address_len);
  dlog("bind(%d, %p, %zi) = %d (%d)\n", socket, address, (size_t)address_len, r,
       E(r));
  return r;
}
